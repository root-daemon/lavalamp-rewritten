...
interface CompletionContext {
  ...
  inputField: { focus: () => void; plainText: string; setText: (text: string) => void; cursorOffset: number };
  accent: () => string;
  nextId: () => string;
}
...
export class CompletionManager {
  ...
  trigger(): void {
    ...
    if (slashIdx !== -1) {
      ...
      if ((slashIdx === 0 || before[slashIdx - 1] === " ") && !afterSlash.includes(" ")) {
        this.completionType = "slash";
        this.completionBaseCol = slashIdx;
        this.completionList = fuzzySearch(afterSlash.toLowerCase(), ALL_SLASH_COMMANDS).map(
          (r) => r.item,
        );
    ...
    if (atMatch) {
      ...
      this.completionBaseCol = before.lastIndexOf("@");
      this.completionList = fuzzySearch((atMatch[1] ?? "").toLowerCase(), this.getFiles()).map(
        (r) => r.item,
      );
      this.completionIndex = 0;
      if (this.completionList.length > 0) {
    ...
    if (hashMatch) {
      ...
      this.completionBaseCol = before.lastIndexOf("#");
      this.completionList = fuzzySearch((hashMatch[1] ?? "").toLowerCase(), this.getSkills()).map(
        (r) => r.item,
      );
      this.completionIndex = 0;
      if (this.completionList.length > 0) {
    ...
    if (this.completing) {this.hide();}
  }

  render(): void {
    for (const child of this.ctx.completionScroll.getChildren()) {child.destroy();}
    ...
    for (let i = 0; i < this.completionList.length; i++) {
      ...
      if (this.completionType === "session") {
        const sess = listSessions().find((s) => s.id === this.completionList[i]);
        if (sess) {
      ...
      if (this.completionType === "slash" || this.completionType === "session") {
        row.add(
          new TextRenderable(this.ctx.renderer, {
            content:
              this.completionType === "slash"
