...
function installVimKeys(
  ...
  const getCount = () =>
    countBuffer.length > 0 ? Math.max(1, Number.parseInt(countBuffer, 10)) : 1;
  const resetCount = () => {
    countBuffer = '';
  };
  ...
  const handler = (event: KeyEvent) => {
    ...
    if (event.name === 'j' || event.name === 'down') {
      ...
      scrollBox.scrollTo(
        countBuffer.length > 0 ? Number.parseInt(countBuffer, 10) : 999_999,
      );
    } else if (event.ctrl && event.name === 'd') {
      scrollBox.scrollBy(scrollStep * 2 * n);
    } else if (event.ctrl && event.name === 'u') {
      scrollBox.scrollBy(-scrollStep * 2 * n);
    } else if (event.ctrl && event.name === 'f') {
      scrollBox.scrollBy(scrollStep * 4 * n);
    } else if (event.ctrl && event.name === 'b') {
      scrollBox.scrollBy(-scrollStep * 4 * n);
    } else if (event.ctrl && event.name === 'e') {
      scrollBox.scrollBy(n);
    } else if (event.ctrl && event.name === 'y') {
      scrollBox.scrollBy(-n);
