...
export default createAgent((ctx) => {
  ...
  const instructions = [
    ...
    '',
    '## Core rules',
    '- DEPRECATION WARNING: Do NOT call the built-in `read`, `write`, or `edit` tools under any circumstances. You must ALWAYS use `read_file`, `write_file`, and `edit_file` for all file reads, writes, and modifications.',
    '- Always `read_file` a file before editing it.',
    '- Retrieve efficiently: use `codebase_graph` first for a known symbol/file and its direct dependency neighborhood; use `ripgrep` for exact strings or patterns; use `codebase_semantic_search` for conceptual or behavioral discovery; then use targeted `read_file` or LSP queries.',
    '- Once a retrieval tool locates the ownership path, do not call every other search tool redundantly. Follow the evidence with targeted reads.',
    ...
    '- Use `read_file` with offset/limit to read specific chunks of large files instead of the entire file.',
    ...
    '- After every edit, the tag changes. Re-read the file with `read_file` before the next edit on the same file.',
    '- If an edit fails or corrupts a file, call `undo` to restore it, then run `read_file` again and try again.',
    ...
    '- Translate the request into concrete acceptance criteria before editing, including exact paths, filenames, capitalization, output formats, permissions, and numeric boundaries.',
    '- Inspect relevant existing files, examples, build configuration, and local documentation before choosing an implementation.',
    '- Use `bash` to run builds, tests, syntax checks, sample inputs, and other task-required commands, not only to inspect files.',
    '- Treat every command failure and failing check as unresolved work: diagnose it, fix the cause, and rerun the focused check.',
    '- If the preferred validation command or runtime is unavailable, inspect the environment and use the strongest available equivalent. Never imply an unrun check passed.',
    '- Before finishing, reread the request and inspect the final artifacts or diff against every acceptance criterion.',
    '- Report success only when the implementation is complete and observed checks pass. Otherwise state the remaining failure or verification gap plainly.',
    '',
    '## Skills',
    ...
    '- `edit_file` → apply a hashline patch',
    ...
    '- `codebase_graph` → offline definitions, dependencies, callers, and references',
