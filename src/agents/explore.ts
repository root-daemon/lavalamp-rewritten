...
export default createAgent((ctx) => {
  ...
  const instructions = [
    ...
    '- Use `codebase_graph` first for a known symbol/file and its direct dependency neighborhood, `ripgrep` for exact strings or patterns, and `codebase_semantic_search` for conceptual or behavioral discovery; then use targeted `read_file` or LSP queries.',
    '- Once a retrieval tool locates the ownership path, do not call every other search tool redundantly.',
